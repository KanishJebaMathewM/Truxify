...
describe('redisLock', () => {
  ...
  it('acquireLock throws LockAcquisitionError when redisClient is null', async () => {
    const { acquireLock, LockAcquisitionError } = await import('../../src/lib/redisLock.js');
    await expect(acquireLock('test-resource', 5000)).rejects.toThrow(LockAcquisitionError);
  });
