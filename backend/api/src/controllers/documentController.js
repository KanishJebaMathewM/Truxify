...
export async function uploadDriverDocument(req, res) {
  try {
    ...
    const { data: existingDocByType, error: checkExistingError } = await client
      .from('driver_documents')
      .select('id, storage_path')
      .eq('driver_id', driverId)
      .eq('document_type', documentType)
    ...
    if (checkExistingError) {
      logger.error('[DocumentController] Failed to check for existing document:', checkExistingError.message);
    ...
    if (existingDocByType?.storage_path) {
      await client.storage
        .from('driver-documents')
        .remove([existingDocByType.storage_path])
        .catch((cleanupErr) => {
          logger.warn('[DocumentController] Failed to delete superseded storage file:', cleanupErr.message);
        });
    }

    return res.status(existingDocByType ? 200 : 201).json({
      success: true,
      document: record,
    });
  } catch (err) {
