...
describe('tracker', () => {
  ...
  describe('handleLocationPing', () => {
    ...
    it('buffers valid location ping', async () => {
      ...
      const bufferArray = __testing.getTelemetryWriteBuffer().toArray();
      expect(bufferArray.length).toBe(1);
      expect(bufferArray[0].lat).toBe(19.076);
      expect(bufferArray[0].lng).toBe(72.877);
    });
  });
  ...
  describe('__testing helpers', () => {
    ...
    it('getTelemetryWriteBuffer returns an array', () => {
      const buf = __testing.getTelemetryWriteBuffer().toArray();
