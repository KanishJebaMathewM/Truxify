...
function run() {
  try {
    // List the last 50 issues authored by the user
    console.log('Fetching issues...');
    const listOutput = execFileSync('gh', ['issue', 'list', '--author', '@me', '--state', 'open', '--limit', '50', '--json', 'number,title'], { encoding: 'utf-8' });
    const existingIssues = JSON.parse(listOutput);
    
    let updatedCount = 0;
    
    for (const existing of existingIssues) {
      // Find matching issue by title
      const matchingIssue = issues.find(i => existing.title.trim() === i.title.trim());
      
      if (matchingIssue) {
        console.log(`Updating issue #${existing.number}: ${existing.title}`);
        
        // Write the body to a temp file to avoid Windows escaping issues
        const tmpFile = path.join(process.cwd(), `tmp_issue_body_${existing.number}.txt`);
        writeFileSync(tmpFile, matchingIssue.body);
        
        try {
          execFileSync('gh', ['issue', 'edit', existing.number.toString(), '--body-file', tmpFile], { encoding: 'utf-8' });
          console.log(`Successfully updated #${existing.number}`);
          updatedCount++;
        } catch(e) {
          console.error(`Failed to update #${existing.number}:`, e.message);
        } finally {
          unlinkSync(tmpFile);
        }
      }
    ...
    console.log(`Successfully updated ${updatedCount} issues.`);
  } catch (err) {
