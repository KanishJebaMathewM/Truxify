...
export class OrderValidationService {
  ...
  async findOrderByIdOrDisplayId(identifier, select = "*") {
    ...
    if (errId)
      throw new DomainError(500, {
        error: "Query failed.",
        details: errId.message,
      });
    if (byId) return byId;
    const { data: byDisplay, error: errDisplay } = await this.supabase
      .from("orders")
      .select(select)
      .eq("order_display_id", identifier)
      .maybeSingle();
    if (errDisplay)
      throw new DomainError(500, {
        error: "Query failed.",
        details: errDisplay.message,
      });
  ...
  assertCustomerOwnership(order, userId) {
    try {
      policy.authorize({ id: userId, role: "customer" }, "order:view-bids", {
        order,
      });
    } catch (err) {
  ...
  assertDriverAssignment(order, driverId) {
    try {
      policy.authorize({ id: driverId, role: "driver" }, "milestone:update", {
        order,
      });
    } catch (err) {
      throw new DomainError(403, {
  ...
  assertOrderAccess(order, user) {
    try {
      ...
      throw new DomainError(403, {
        error:
          "Access Denied: You do not own or are not assigned to this order.",
      });
    }
  }
  ...
  assertOrderStatus(order, allowedStatuses, errorMsg) {
    if (!allowedStatuses.includes(order.status)) {
      throw new DomainError(409, {
        error:
          errorMsg ||
          `Order status '${order.status}' does not allow this operation.`,
      });
    }
  }
  ...
  assertNotTerminalStatus(order) {
    if (["delivered", "cancelled", "payment_released"].includes(order.status)) {
      throw new DomainError(409, {
        error:
          "Order was already cancelled, delivered, or payment released. Cannot cancel.",
      });
    }
  }
  ...
  assertEscrowState(order, allowedStates, errorMsg) {
    if (!allowedStates.includes(order.escrow_status)) {
      throw new DomainError(400, {
        error:
          errorMsg ||
          `Escrow status '${order.escrow_status}' does not allow this operation.`,
      });
    }
  }
  ...
  async assertLoadOfferAvailable(loadOfferId) {
    ...
    if (offer.status !== "available") {
      throw new DomainError(410, {
        error: "Load is no longer available for bidding.",
      });
    }
    return offer;
  }
  ...
  assertNotOwnLoad(offerCustomerId, userId) {
    ...
    try {
      ...
      if (
        err.code === "OWN_LOAD_VIOLATION" ||
        err.message?.includes("own load")
      ) {
        throw new DomainError(403, {
          error: "You cannot bid on your own load offer",
        });
      }
      this.logger.error(
        { err, userId, offerCustomerId },
        "Policy authorization failed in assertNotOwnLoad",
      );
      throw new DomainError(500, {
  ...
  async assertTruckAssigned(driverId) {
    const { data: driverDetails } = await this.supabase
      .from("driver_details")
      .select("truck_id")
      .eq("user_id", driverId)
      .maybeSingle();
    if (!driverDetails?.truck_id) {
      throw new DomainError(400, {
        error:
          "You must assign a valid truck to your profile before bidding on loads",
      });
    }

    const { data: truck } = await this.supabase
      .from("trucks")
      .select("id")
  ...
  async assertNoDuplicateRating(orderDisplayId, customerId) {
    ...
    if (existingRating) {
      throw new DomainError(409, {
        error: "A rating has already been submitted for this order.",
      });
    }
  }
  ...
  assertRatingDeliverable(order) {
    if (!["delivered", "payment_released"].includes(order.status)) {
      throw new DomainError(400, {
        error: "Order must be delivered before a rating can be submitted.",
      });
    }
    if (!order.driver_id) {
      throw new DomainError(400, {
  ...
  async assertDeliveryNotVerified(orderId) {
    const { data: otpCheck } = await this.supabase
      .from("delivery_otps")
      .select("id")
      .eq("order_id", orderId)
      .eq("verified", true)
    ...
    if (otpCheck) {
      throw new DomainError(409, {
        error: "Cannot cancel: delivery OTP has already been verified.",
      });
    }
  }
  ...
  assertMilestoneNotDuplicate(entry) {
    if (entry.completed) {
      throw new DomainError(409, {
        error: `Milestone "${entry.milestone}" has already been completed.`,
      });
    }
  }
  ...
  assertMilestoneSequence(timeline, milestone, lastCompletedSortOrder) {
    const entry = timeline.find((t) => t.milestone === milestone);
    const nextExpected = timeline.find(
      (t) => !t.completed && t.sort_order > lastCompletedSortOrder,
    );
  ...
  assertChangeDropAllowed(order) {
    const escrowInFlight =
      order.escrow_status === "funding" || order.escrow_status === "funded";
    if (escrowInFlight || order.status !== "pending") {
      ...
      throw new DomainError(409, {
        ...
        recovery:
          "Cancel this order to receive a refund, then rebook with the correct destination.",
      });
    }
  }
  ...
  assertHasWeight(order) {
    if (order.weight_tonnes == null) {
      throw new DomainError(500, {
        error: "Data inconsistency: Order is missing weight_tonnes.",
      });
    }
  }
  ...
  async assertHosCompliant(driverId) {
    const { data: driver, error } = await this.supabase
      .from("driver_details")
      .select(
        "accumulated_driving_minutes, accumulated_on_duty_minutes, hos_status",
      )
      .eq("user_id", driverId)
      .maybeSingle();
    ...
    if (error) {
      throw new DomainError(500, {
        error: "Failed to verify driver HoS status.",
    ...
    if (driver) {
      ...
      if (drivingHours >= 11 || onDutyHours >= 14) {
        throw new DomainError(403, {
          error:
            "HoS Limit Exceeded: You have reached your maximum legal driving or on-duty hours for this shift. You must take a mandatory rest break before bidding on new loads.",
        });
      }
    }
  }
