...
class RouteOptimizer(_BaseRouteOptimizer):
    """Allow live rerouting to start from the vehicle's current graph node."""
    ...
    def real_time_update(
        ...
        self._apply_traffic_to_route(updated_route, new_traffic_data)
        if not changed or current_node is None:
            return updated_route
        ...
        if current_node not in graph:
            ...
            if logger:
                logger.warning(
                    "Current vehicle node %s is not present in the routing graph",
                    current_node,
                )
            return updated_route
